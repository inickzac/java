...
import PortConcur.SeaPort.NotEnoughGoodsException;
...
public class Berth extends Thread {
	...
	public void run() {
		try {
			while (true) {
				ship = port.getShipInQueue();
				System.out
						.print("Корабль " + ship.getShipName() + " Зашел на причал " + name + " Порта " + port.getNane() + "\n");
				...
				System.out.print(
						"Корабль " + ship.getShipName() + " Вышел из причала " + name + " Порта " + port.getNane() + "\n");
				ship = null;
	...
	private void unloadShip(Ship ship)
			throws InterruptedException, NotEnoughGoodsException, PortConcur.Ship.NotEnoughGoodsException {
	...
	private void recordTheWrongParking(Ship ship) throws IOException {
		...
		fw.write(ship.getShipName() +" Превысил время нахождения на порту\n");
		fw.close();
	}
